...
function Stuffing:CreateBagFrame(w)
	...
	local function bagUpdate(f, ...)
		if w == "Bank" then
			if not C["chat"].background then
				f:Point("BOTTOMLEFT", TukuiInfoLeft, "TOPLEFT", 0, 3)
			else
				f:Point("BOTTOMLEFT", TukuiTabsLeftBackground, "TOPLEFT", 0, 3)
			end
		else
			if not C["actionbar"].enable then
				if not C["chat"].background then
					f:Point("BOTTOMLEFT", TukuiInfoRight, "TOPLEFT", 0, 3)
				else
					f:Point("BOTTOMRIGHT", TukuiTabsRightBackground, "TOPRIGHT", 0, 3)
				end
			else
				if not C["chat"].background then
					f:Point("BOTTOMLEFT", TukuiInfoRight, "TOPLEFT", 0, 3)
				elseif HasPetUI() then
					if C["actionbar"].sidebarWidth >= 1 then
						f:Point("BOTTOMRIGHT", TukuiTabsRightBackground, "TOPRIGHT", 0, 3)
					else
						f:Point("BOTTOMRIGHT", TukuiPetBar, "TOPRIGHT", 0, 3)
					end
	...
	if w == "Bank" then
		f:Point("BOTTOMLEFT", TukuiInfoLeft, "TOPLEFT", 0, 140)
	else
		f:Point("BOTTOMRIGHT", TukuiInfoRight, "TOPRIGHT", 0, 178)
	end
...
function Stuffing:InitBags()
	...
	gold:Point("RIGHT", f.b_close, "LEFT", -5, 0)
...
function Stuffing:PLAYER_ENTERING_WORLD()
	...
	ContainerFrame1:Point("BOTTOMRIGHT", TukuiInfoRight, "TOPLEFT", 0, 2)  -- move the keyring so it doesnt open behind the bags
