...
if C["unitframes"].style ~= "Ex2" then return end
...
local function Shared(self, unit)
	...
	if (unit == "player" or unit == "target") then
		...
		if unit == "player" then
			if C.unitframes.classicon then
				classicon:CreatePanel("Default", 33, 33, "TOPLEFT", health, "TOPRIGHT", 5,2)
			else
				classicon:CreatePanel("Default", 33, 33, "TOPRIGHT", health, "TOPLEFT", -5,2)
			end
		elseif unit == "target" then
			if C.unitframes.classicon then
				classicon:CreatePanel("Default", 33, 33, "TOPRIGHT", health, "TOPLEFT", -5,2)
		...
		if (unit == "player") then
			...
			if T.level ~= MAX_PLAYER_LEVEL then
				...
				Experience:Size(TukuiMinimap:GetWidth() + 27, 5)
				Experience:Point("TOPLEFT", TukuiInfoLeftMinimap, "BOTTOMLEFT", 2, -5)
				...
				Resting:SetWidth(28)
				Resting:SetPoint("BOTTOMRIGHT", self, "TOPLEFT", 11, -4)
				Resting:SetTexture([=[Interface\CharacterFrame\UI-StateIcon]=])
				Resting:SetTexCoord(0, 0.5, 0, 0.421875)
				self.Resting = Resting
				
				local ExperienceFrame = CreateFrame("Frame", nil, self.Experience)
				ExperienceFrame:SetPoint("TOPLEFT", -2, 2)
				ExperienceFrame:SetPoint("BOTTOMRIGHT", 2, -2)
			...
			if T.level == MAX_PLAYER_LEVEL then
				...
				Reputation:Size(TukuiMinimap:GetWidth() + 27, 5)
				Reputation:Point("TOPLEFT", TukuiInfoLeftMinimap, "BOTTOMLEFT", 2, -5)
				...
				ReputationFrame:SetPoint("TOPLEFT", -2, 2)
				ReputationFrame:SetPoint("BOTTOMRIGHT", 2, -2)
			...
			if C["unitframes"].classbar then
				if T.myclass == "DRUID" and C["unitframes"].druid then
					...
					eclipseBar:Point("BOTTOM", self, "TOP", 0, 5)
				...
				if T.myclass == "DEATHKNIGHT" then
					...
					Runes:Point("BOTTOMLEFT", self, "TOPLEFT", 0, 5)
					Runes:Size(218, 2)
					...
					for i = 1, 6 do
						...
						if (i == 1) then
							Runes[i]:Point("BOTTOMLEFT", self, "TOPLEFT", 0, 5)
						else
		...
		if (C["unitframes"].unitcastbar == true) then
			...
			if unit == "player" then
				...
				castbar:Point("BOTTOM", TukuiBar1, "TOP", 0, 4)
			elseif unit == "target" then
				if C["unitframes"].cbicons == true then
	...
	if (unit and unit:find("arena%d") and C["arena"].unitframes == true) or (unit and unit:find("boss%d") and C["unitframes"].showboss == true) then
		...
		if (C.arena.unitframes) and (unit and unit:find('arena%d')) then
			...
			Trinketbg:SetHeight(29)
			Trinketbg:SetWidth(29)
...
tot:SetPoint("BOTTOM", InvTukuiActionBarBackground, "TOP", 0,117)
tot:Size(128, 24)

-- pet
local pet = oUF:Spawn('pet', "TukuiPet")
pet:SetPoint("BOTTOMRIGHT", TukuiPlayer, "TOPLEFT", -7,5)
if C["unitframes"].extendedpet == true then
	pet:Size(100, 28)
else
	pet:Size(54, 16)
